...
const OFFLINE_MAPS_PAGE = path.resolve("dist/public/shop/p/app-store-offline-maps-chinese.html");
...
describe("Render edge dist/index.js shop pretty URLs", () => {
  ...
  it("serves /shop/p/app-store-hidden-apps-chinese.html with 200 from dist/public", async () => {
    expect(fs.existsSync(HIDDEN_APPS_PAGE)).toBe(true);
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:${port}/shop/p/app-store-hidden-apps-chinese.html`
      );
      expect(res.status).toBe(200);
      expect(res.headers.get("content-type")).toContain("text/html");
      const html = await res.text();
      expect(html).toContain("Hidden Apps");
      expect(html).toContain("Remove from Home Screen");
      expect(html).toContain("隐藏应用");
    } finally {
      child.kill("SIGTERM");
      await new Promise<void>((resolve) => {
        child.on("exit", () => resolve());
  ...
  it("serves /shop/p/app-store-offline-maps-chinese.html with 200 from dist/public", async () => {
    expect(fs.existsSync(OFFLINE_MAPS_PAGE)).toBe(true);
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:${port}/shop/p/app-store-offline-maps-chinese.html`
      );
      expect(res.status).toBe(200);
      expect(res.headers.get("content-type")).toContain("text/html");
      const html = await res.text();
      expect(html).toContain("Download Offline Maps");
      expect(html).toContain("离线地图");
    } finally {
      child.kill("SIGTERM");
      await new Promise<void>((resolve) => {
        child.on("exit", () => resolve());
  ...
  it("serves /shop/p/app-store-ev-routing-chinese.html with 200 from dist/public", async () => {
    expect(fs.existsSync(EV_ROUTING_PAGE)).toBe(true);
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:${port}/shop/p/app-store-ev-routing-chinese.html`
      );
      expect(res.status).toBe(200);
      expect(res.headers.get("content-type")).toContain("text/html");
      const html = await res.text();
      expect(html).toContain("EV Routing");
      expect(html).toContain("电动车路线");
  ...
  it("serves /shop/p/app-store-indoor-maps-chinese.html with 200 from dist/public", async () => {
    expect(fs.existsSync(INDOOR_MAPS_PAGE)).toBe(true);
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:${port}/shop/p/app-store-indoor-maps-chinese.html`
      );
      expect(res.status).toBe(200);
      expect(res.headers.get("content-type")).toContain("text/html");
      const html = await res.text();
  ...
  it("serves /shop/p/app-store-people-occlusion-chinese.html with 200 from dist/public", async () => {
    expect(fs.existsSync(PEOPLE_OCCLUSION_PAGE)).toBe(true);
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:${port}/shop/p/app-store-people-occlusion-chinese.html`
      );
      expect(res.status).toBe(200);
      expect(res.headers.get("content-type")).toContain("text/html");
      const html = await res.text();
