...
function resolveSignupTrialDays(raw2 = process.env.SIGNUP_TRIAL_DAYS) {
...
async function findUserByEmail(email) {
  ...
  if (!db) return null;
  const normalized = normalizeEmail(email);
  const [row] = await db.select().from(users).where(eq12(users.email, normalized)).limit(1);
  return row ?? null;
}
async function registerWithEmail(input) {
  ...
  if (existing) {
    ...
    }).where(eq12(users.id, existing.id));
    const [updated] = await db.select().from(users).where(eq12(users.id, existing.id)).limit(1);
  ...
  const [created] = await db.select().from(users).where(eq12(users.openId, openId)).limit(1);
...
async function loginWithEmail(input) {
  ...
  const db = await getDb();
  if (db) {
    await db.update(users).set({ lastSignedIn: /* @__PURE__ */ new Date() }).where(eq12(users.id, user.id));
  }
...
init_db();
init_schema();
import { eq as eq13, desc as desc12, and as and11, sql as sql12 } from "drizzle-orm";
...
import { eq as eq3, desc as desc3, and as and3, or, isNull, sql as sql3, inArray } from "drizzle-orm";
...
var notificationRouter = router({
  ...
  list: protectedProcedure.input(z3.object({
    ...
    const broadcastNotifs = await db.select().from(notifications).where(
      and3(
        eq3(notifications.isBroadcast, true),
    ...
    if (broadcastIds.length > 0) {
      const receipts = await db.select({ notificationId: broadcastReadReceipts.notificationId }).from(broadcastReadReceipts).where(
        and3(
          eq3(broadcastReadReceipts.userId, userId),
  ...
  markRead: protectedProcedure.input(z3.object({ id: z3.number() })).mutation(async ({ ctx, input }) => {
    ...
    const userId = ctx.user.id;
    const [notif] = await db.select().from(notifications).where(eq3(notifications.id, input.id)).limit(1);
    ...
    if (notif.isBroadcast) {
      const existing = await db.select().from(broadcastReadReceipts).where(
        and3(
          eq3(broadcastReadReceipts.notificationId, input.id),
          eq3(broadcastReadReceipts.userId, userId)
        )
      ).limit(1);
      ...
      await db.update(notifications).set({ isRead: true }).where(eq3(notifications.id, input.id));
    }
    return { success: true };
  }),
  /**
  ...
  markAllRead: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    await db.update(notifications).set({ isRead: true }).where(
      and3(eq3(notifications.userId, userId), eq3(notifications.isRead, false))
    );
  ...
  delete: protectedProcedure.input(z3.object({ id: z3.number() })).mutation(async ({ ctx, input }) => {
    ...
    if (notif.userId !== userId) throw new TRPCError3({ code: "FORBIDDEN" });
    await db.delete(notifications).where(eq3(notifications.id, input.id));
    return { success: true };
  }),
  // ========== Admin endpoints ==========
  /**
  ...
  adminDelete: protectedProcedure.input(z3.object({ id: z3.number() })).mutation(async ({ ctx, input }) => {
    ...
    if (!db) throw new TRPCError3({ code: "INTERNAL_SERVER_ERROR" });
    await db.delete(notifications).where(eq3(notifications.id, input.id));
    await db.delete(broadcastReadReceipts).where(eq3(broadcastReadReceipts.notificationId, input.id));
    return { success: true };
  }),
  /**
...
async function handleSubscriptionUpdate(subscription) {
  ...
  }).where(eq4(memberships.stripeCustomerId, customerId));
...
async function handleSubscriptionCanceled(subscription) {
  ...
  const [cancelledMembership] = await db.select().from(memberships).where(eq4(memberships.stripeCustomerId, customerId)).limit(1);
...
async function handleInvoicePaid(invoice) {
  ...
  const db = await getDb();
  if (!db) return;
  const [membership] = await db.select().from(memberships).where(eq4(memberships.stripeCustomerId, customerId)).limit(1);
  if (membership) {
    const orderNo = `ORD${Date.now()}${nanoid(6)}`;
    const [orderResult] = await db.insert(orders).values({
...
\u8BF7\u57FA\u4E8E\u4EE5\u4E0A\u724C\u9762\u548C\u724C\u4F4D\uFF0C\u63D0\u4F9B\u4E13\u4E1A\u3001\u6DF1\u5165\u7684\u5854\u7F57\u89E3\u8BFB\u3002`;
  ...
  getHistory: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return [];
    const readings = await db.select().from(tarotReadings).where(eq5(tarotReadings.userId, ctx.user.id)).orderBy(desc4(tarotReadings.createdAt)).limit(20);
    return readings;
  })
});
...
init_db();
init_schema();
import { eq as eq6, desc as desc5, and as and5, sql as sql4 } from "drizzle-orm";
...
${structuredPrompt}${hourNote}`;
    ...
    if (db) {
      ...
      if (ctx.user?.id) {
        const [existingGrowth] = await db.select().from(userGrowth).where(eq6(userGrowth.userId, ctx.user.id)).limit(1);
        if (existingGrowth) {
          ...
          }).where(eq6(userGrowth.userId, ctx.user.id));
        }
      }
    }
    return {
...
\u8BF7\u5F15\u7528\u5177\u4F53\u7684\u547D\u76D8\u6570\u636E\u56DE\u7B54\u95EE\u9898\u3002\u8BED\u6C14\u6E29\u6696\u9F13\u52B1\uFF0C\u63D0\u4F9B\u53EF\u64CD\u4F5C\u7684\u5EFA\u8BAE\u3002\u56DE\u7B54\u63A7\u5236\u5728300\u5B57\u4EE5\u5185\u4F46\u8981\u6709\u5B9E\u8D28\u5185\u5BB9\u3002`;
  ...
  getHistory: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return [];
    const readings = await db.select().from(baziReadings).where(eq6(baziReadings.userId, ctx.user.id)).orderBy(desc5(baziReadings.createdAt)).limit(20);
    return readings;
  }),
  ...
  exportSingle: protectedProcedure.input(z5.object({ readingId: z5.number() })).mutation(async ({ input, ctx }) => {
    ...
    const readings = await db.select().from(baziReadings).where(and5(eq6(baziReadings.id, input.readingId), eq6(baziReadings.userId, ctx.user.id))).limit(1);
  ...
  exportBatch: protectedProcedure.input(z5.object({
    ...
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    const readings = await db.select().from(baziReadings).where(eq6(baziReadings.userId, ctx.user.id)).orderBy(desc5(baziReadings.createdAt));
  ...
  getDaily: publicProcedure.input(z6.object({
    ...
    if (db) {
      const [cached] = await db.select().from(horoscopes).where(and6(
        eq7(horoscopes.zodiacSign, cacheSignKey),
        eq7(horoscopes.periodType, "daily"),
        eq7(horoscopes.periodDate, today)
      )).limit(1);
...
init_db();
init_schema();
import { eq as eq8, desc as desc7, and as and7, sql as sql6 } from "drizzle-orm";
...
\u8BF7\u57FA\u4E8E\u68A6\u5883\u5185\u5BB9\u548C\u4E0A\u8FF0\u5206\u6790\u5F15\u64CE\u6570\u636E\uFF0C\u63D0\u4F9B\u5168\u9762\u768410\u7EF4\u5EA6\u6DF1\u5EA6\u68A6\u5883\u5206\u6790\u3002\u6BCF\u4E2A\u7EF4\u5EA6\u5FC5\u987B\u5145\u5B9E\u4E14\u6709\u6D1E\u5BDF\u529B\u3002`;
  ...
  getHistory: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return [];
    const dreams = await db.select().from(dreamRecords).where(eq8(dreamRecords.userId, ctx.user.id)).orderBy(desc7(dreamRecords.createdAt)).limit(50);
    return dreams;
  }),
  getById: protectedProcedure.input(z7.object({ id: z7.number() })).query(async ({ input, ctx }) => {
    const db = await getDb();
    if (!db) return null;
    const [dream] = await db.select().from(dreamRecords).where(and7(eq8(dreamRecords.id, input.id), eq8(dreamRecords.userId, ctx.user.id))).limit(1);
    return dream || null;
  }),
  ...
  exportBatch: protectedProcedure.input(z7.object({
    ...
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    const dreams = await db.select().from(dreamRecords).where(eq8(dreamRecords.userId, ctx.user.id)).orderBy(desc7(dreamRecords.createdAt)).limit(input.limit || 50);
  ...
  search: protectedProcedure.input(z7.object({
    ...
    const db = await getDb();
    if (!db) return [];
    const allDreams = await db.select().from(dreamRecords).where(eq8(dreamRecords.userId, ctx.user.id)).orderBy(desc7(dreamRecords.createdAt)).limit(200);
  ...
  getAllTags: protectedProcedure.query(async ({ ctx }) => {
    ...
    const records = await db.select({ tags: dreamRecords.tags }).from(dreamRecords).where(eq8(dreamRecords.userId, ctx.user.id));
    const tagCounts = {};
  ...
  getStats: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return null;
    const records = await db.select().from(dreamRecords).where(eq8(dreamRecords.userId, ctx.user.id));
  ...
  updateTags: protectedProcedure.input(z7.object({
    ...
    await db.update(dreamRecords).set({ tags: input.tags }).where(
      and7(
        eq8(dreamRecords.id, input.dreamId),
        eq8(dreamRecords.userId, ctx.user.id)
      )
    );
    return { success: true };
  })
...
init_db();
init_schema();
import { eq as eq9, and as and8, desc as desc8, sql as sql7, count } from "drizzle-orm";
...
var referralRouter = router({
  ...
  getMyReferral: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [user] = await db.select().from(users).where(eq9(users.id, ctx.user.id)).limit(1);
    let referralCode = user?.referralCode;
    if (!referralCode) {
      referralCode = generateReferralCode();
      let attempts = 0;
      while (attempts < 5) {
        const [existing] = await db.select().from(users).where(eq9(users.referralCode, referralCode)).limit(1);
        if (!existing) break;
        referralCode = generateReferralCode();
        attempts++;
      }
      await db.update(users).set({ referralCode }).where(eq9(users.id, ctx.user.id));
    }
    const [totalResult] = await db.select({ count: count() }).from(referrals).where(eq9(referrals.referrerId, ctx.user.id));
    const [completedResult] = await db.select({ count: count() }).from(referrals).where(
      and8(
        eq9(referrals.referrerId, ctx.user.id),
        eq9(referrals.status, "rewarded")
      )
    );
    const [rewardsResult] = await db.select({ total: sql7`COALESCE(SUM(${referralRewards.creditsAmount}), 0)` }).from(referralRewards).where(eq9(referralRewards.userId, ctx.user.id));
    return {
      code: referralCode,
  ...
  getReferralHistory: protectedProcedure.query(async ({ ctx }) => {
    ...
    }).from(referrals).leftJoin(users, eq9(referrals.referredId, users.id)).where(eq9(referrals.referrerId, ctx.user.id)).orderBy(desc8(referrals.createdAt)).limit(50);
  ...
  getMyRewards: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return [];
    return db.select().from(referralRewards).where(eq9(referralRewards.userId, ctx.user.id)).orderBy(desc8(referralRewards.createdAt)).limit(50);
  }),
  ...
  validateCode: publicProcedure.input(z8.object({ code: z8.string().min(1) })).query(async ({ input }) => {
    ...
    const [user] = await db.select({ id: users.id, name: users.name }).from(users).where(eq9(users.referralCode, input.code.toUpperCase())).limit(1);
    return {
      valid: !!user,
  ...
  processReferral: protectedProcedure.input(z8.object({ referralCode: z8.string().min(1) })).mutation(async ({ ctx, input }) => {
    ...
    const [referrer] = await db.select().from(users).where(eq9(users.referralCode, code)).limit(1);
    ...
    const [existing] = await db.select().from(referrals).where(eq9(referrals.referredId, ctx.user.id)).limit(1);
    ...
    await db.update(users).set({ referredBy: referrer.id }).where(eq9(users.id, ctx.user.id));
    ...
    }).where(eq9(referrals.id, Number(referralId)));
    createNotification({
      userId: referrer.id,
      type: "system",
  ...
  getLeaderboard: publicProcedure.query(async () => {
    ...
    const results = await db.select({
      ...
      count: count(),
      name: users.name
    }).from(referrals).leftJoin(users, eq9(referrals.referrerId, users.id)).where(eq9(referrals.status, "rewarded")).groupBy(referrals.referrerId, users.name).orderBy(desc8(count())).limit(10);
...
\u8BF7\u63D0\u4F9B\u4E13\u4E1A\u3001\u6DF1\u5165\u7684\u5408\u76D8\u5206\u6790\uFF0C\u5305\u542B\u5168\u90E810\u4E2A\u7EF4\u5EA6\uFF0C\u6700\u540E\u9644\u4E0AJSON\u8BC4\u5206\u5757\u3002`;
    ...
    if (db) {
      ...
      if (ctx.user?.id) {
        const [existingGrowth] = await db.select().from(userGrowth).where(eq10(userGrowth.userId, ctx.user.id)).limit(1);
        if (existingGrowth) {
          await db.update(userGrowth).set({
            intimateRelationships: sql8`${userGrowth.intimateRelationships} + 3`,
            totalPoints: sql8`${userGrowth.totalPoints} + 5`
          }).where(eq10(userGrowth.userId, ctx.user.id));
        }
      }
    }
    return {
  ...
  getHistory: protectedProcedure.query(async ({ ctx }) => {
    ...
    }).from(compatibilityReports).where(eq10(compatibilityReports.userId, ctx.user.id)).orderBy(desc9(compatibilityReports.createdAt)).limit(20);
    return reports;
  })
});
...
import { and as and10, desc as desc11, eq as eq11, sql as sql10 } from "drizzle-orm";
init_db();
init_schema();
...
async function ensureAccessCodeTables() {
  ...
  `);
  const [existing] = await db.select({ id: accessCodes.id }).from(accessCodes).where(eq11(accessCodes.code, DEFAULT_FRIEND_CODE)).limit(1);
  if (!existing) {
...
function normalizeCode(raw2) {
  return raw2.trim().toUpperCase().replace(/\s+/g, "");
}
...
async function activateMembership(userId, type, paymentMethod, transactionId) {
  ...
  await db.update(memberships).set({ status: "cancelled", autoRenew: false }).where(and10(eq11(memberships.userId, userId), eq11(memberships.status, "active")));
  const now = /* @__PURE__ */ new Date();
  await db.insert(memberships).values({
    userId,
    type,
...
var accessCodeRouter = router({
  ...
  preview: publicProcedure.input(z11.object({ code: z11.string().min(3).max(40) })).query(async ({ input }) => {
    ...
    const [row] = await db.select().from(accessCodes).where(eq11(accessCodes.code, code)).limit(1);
  ...
  redeem: protectedProcedure.input(z11.object({ code: z11.string().min(3).max(40) })).mutation(async ({ ctx, input }) => {
    ...
    const [already] = await db.select({ id: accessCodeRedemptions.id }).from(accessCodeRedemptions).where(
      and10(
        eq11(accessCodeRedemptions.codeId, row.id),
        eq11(accessCodeRedemptions.userId, ctx.user.id)
      )
    ).limit(1);
    if (already) {
    ...
    const updated = await db.update(accessCodes).set({
      ...
      and10(
        eq11(accessCodes.id, row.id),
        eq11(accessCodes.status, "active"),
        eq11(accessCodes.usedCount, row.usedCount)
      )
    );
  ...
  adminDisable: protectedProcedure.input(z11.object({ id: z11.number().int().positive() })).mutation(async ({ ctx, input }) => {
    ...
    const db = await getDb();
    if (!db) throw new TRPCError4({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
    await db.update(accessCodes).set({ status: "disabled" }).where(eq11(accessCodes.id, input.id));
    return { success: true };
  }),
  ...
  adminEnable: protectedProcedure.input(
    ...
    await db.update(accessCodes).set(patch).where(eq11(accessCodes.id, input.id));
    return { success: true };
  })
});
...
var appRouter = router({
  ...
  admin: router({
    overview: protectedProcedure.query(async ({ ctx }) => {
      ...
      const [
        ...
        db.select().from(memberships).where(eq13(memberships.status, "active")),
        db.select({ c: sql12`count(*)` }).from(memberships).where(eq13(memberships.status, "expired")),
        db.select({ c: sql12`count(*)` }).from(contactSubmissions),
        db.select({ c: sql12`count(*)` }).from(chatSessions).where(eq13(chatSessions.status, "waiting")),
        db.select({
          id: users.id,
          name: users.name,
          email: users.email,
  ...
  growth: router({
    getProgress: protectedProcedure.query(async ({ ctx }) => {
      const db = await getDb();
      if (!db) return null;
      const [progress] = await db.select().from(userGrowth).where(eq13(userGrowth.userId, ctx.user.id)).limit(1);
      if (!progress) {
        await db.insert(userGrowth).values({
          userId: ctx.user.id,
          selfAwareness: 0,
    ...
    addPoints: protectedProcedure.input(z12.object({
      ...
      }).where(eq13(userGrowth.userId, ctx.user.id));
      return { success: true };
    }),
  ...
  community: router({
    getPosts: publicProcedure.input(z12.object({
      ...
      const whereClause = type ? and11(eq13(communityPosts.status, "published"), eq13(communityPosts.type, type)) : eq13(communityPosts.status, "published");
      const posts = await db.select({
        ...
        userName: users.name
      }).from(communityPosts).leftJoin(users, eq13(communityPosts.userId, users.id)).where(whereClause).orderBy(desc12(communityPosts.createdAt)).limit(limit).offset(offset);
      ...
      const activeMemberships = userIds.length > 0 ? await db.select({ userId: memberships.userId }).from(memberships).where(and11(
        eq13(memberships.status, "active"),
    ...
    likePost: protectedProcedure.input(z12.object({ postId: z12.number() })).mutation(async ({ input, ctx }) => {
      ...
      const [existing] = await db.select().from(postLikes).where(and11(
        eq13(postLikes.postId, input.postId),
        eq13(postLikes.userId, ctx.user.id)
      )).limit(1);
      if (existing) {
        await db.delete(postLikes).where(eq13(postLikes.id, existing.id));
        await db.update(communityPosts).set({ likesCount: sql12`${communityPosts.likesCount} - 1` }).where(eq13(communityPosts.id, input.postId));
        return { liked: false };
      } else {
        ...
        await db.update(communityPosts).set({ likesCount: sql12`${communityPosts.likesCount} + 1` }).where(eq13(communityPosts.id, input.postId));
        const [likedPost] = await db.select({ userId: communityPosts.userId }).from(communityPosts).where(eq13(communityPosts.id, input.postId)).limit(1);
    ...
    getComments: publicProcedure.input(z12.object({
      ...
      const rows = await db.select({
        ...
        userName: users.name
      }).from(postComments).leftJoin(users, eq13(postComments.userId, users.id)).where(eq13(postComments.postId, input.postId)).orderBy(desc12(postComments.createdAt)).limit(input.limit);
      return rows.map((r) => ({
        ...r,
    ...
    addComment: protectedProcedure.input(z12.object({
      ...
      const [post] = await db.select({ id: communityPosts.id, userId: communityPosts.userId }).from(communityPosts).where(eq13(communityPosts.id, input.postId)).limit(1);
      if (!post) {
      ...
      await db.update(communityPosts).set({ commentsCount: sql12`${communityPosts.commentsCount} + 1` }).where(eq13(communityPosts.id, input.postId));
  ...
  payment: router({
    ...
    getMembership: protectedProcedure.query(async ({ ctx }) => {
      ...
      const [membership] = await db.select().from(memberships).where(and11(
        eq13(memberships.userId, ctx.user.id),
        eq13(memberships.status, "active")
    ...
    getOrders: protectedProcedure.query(async ({ ctx }) => {
      const db = await getDb();
      if (!db) return [];
      const userOrders = await db.select().from(orders).where(eq13(orders.userId, ctx.user.id)).orderBy(desc12(orders.createdAt)).limit(50);
      return userOrders;
    }),
    getCharityDonations: protectedProcedure.query(async ({ ctx }) => {
      const db = await getDb();
      if (!db) return [];
      const donations = await db.select().from(charityDonations).where(eq13(charityDonations.userId, ctx.user.id)).orderBy(desc12(charityDonations.createdAt)).limit(50);
      return donations;
    }),
    ...
    adminGrantMembership: protectedProcedure.input(z12.object({
      ...
      let targetUser;
      if (input.userId) {
        const [u] = await db.select({ id: users.id, name: users.name, email: users.email }).from(users).where(eq13(users.id, input.userId)).limit(1);
        targetUser = u;
      } else if (input.email) {
        const [u] = await db.select({ id: users.id, name: users.name, email: users.email }).from(users).where(eq13(users.email, input.email)).limit(1);
        targetUser = u;
      }
      if (!targetUser) {
      ...
      await db.update(memberships).set({ status: "cancelled", autoRenew: false }).where(
        and11(
          eq13(memberships.userId, targetUser.id),
          eq13(memberships.status, "active")
        )
      );
      const now = /* @__PURE__ */ new Date();
    ...
    adminRevokeMembership: protectedProcedure.input(z12.object({
      ...
      await db.update(memberships).set({ status: "cancelled", autoRenew: false }).where(
        and11(
          eq13(memberships.userId, input.userId),
          eq13(memberships.status, "active")
        )
      );
      await createNotification({
    ...
    checkSubmitted: publicProcedure.input(z12.object({
      ...
      if (ctx.user?.id && input.sourceId) {
        const [existing] = await db.select().from(userFeedbacks).where(and11(
          eq13(userFeedbacks.userId, ctx.user.id),
          eq13(userFeedbacks.sourceType, input.sourceType),
          eq13(userFeedbacks.sourceId, input.sourceId)
        )).limit(1);
        return { submitted: !!existing };
      }
      if (input.sessionId) {
        const [existing] = await db.select().from(userFeedbacks).where(and11(
          eq13(userFeedbacks.sessionId, input.sessionId),
          eq13(userFeedbacks.sourceType, input.sourceType)
        )).limit(1);
        return { submitted: !!existing };
      }
...
${input.message}`
    ...
    updateStatus: protectedProcedure.input(z12.object({
      ...
      await db.update(contactSubmissions).set(updateData).where(eq13(contactSubmissions.id, input.id));
      return { success: true };
    })
  }),
  // 在线客服聊天路由
...
\u8BF7\u524D\u5F80\u7BA1\u7406\u540E\u53F0\u5904\u7406`
    ...
    sendMessage: publicProcedure.input(z12.object({
      ...
      const db = await getDb();
      if (!db) throw new Error("\u6570\u636E\u5E93\u8FDE\u63A5\u5931\u8D25");
      const sessions = await db.select().from(chatSessions).where(eq13(chatSessions.sessionId, input.sessionId)).limit(1);
      ...
      await db.update(chatSessions).set({ lastMessageAt: /* @__PURE__ */ new Date() }).where(eq13(chatSessions.sessionId, input.sessionId));
    ...
    getMessages: publicProcedure.input(z12.object({
      ...
      const db = await getDb();
      if (!db) return [];
      let query = db.select().from(chatMessages).where(eq13(chatMessages.sessionId, input.sessionId)).orderBy(chatMessages.createdAt).limit(100);
      if (input.lastId) {
        query = db.select().from(chatMessages).where(and11(
          eq13(chatMessages.sessionId, input.sessionId),
    ...
    closeSession: publicProcedure.input(z12.object({
      ...
      }).where(eq13(chatSessions.sessionId, input.sessionId));
      await db.insert(chatMessages).values({
        sessionId: input.sessionId,
        senderType: "system",
        senderName: "\u7CFB\u7EDF",
    ...
    adminGetSessions: protectedProcedure.input(z12.object({
      ...
      return await db.select().from(chatSessions).where(eq13(chatSessions.status, input.status)).orderBy(desc12(chatSessions.updatedAt)).limit(100);
    }),
    // 管理员发送消息
    ...
    adminGetStats: protectedProcedure.query(async ({ ctx }) => {
      ...
      for (const session of allSessions) {
        if (session.status !== "waiting" && session.createdAt) {
          const messages = await db.select().from(chatMessages).where(eq13(chatMessages.sessionId, session.sessionId)).orderBy(chatMessages.createdAt);
...
function serveStatic(app) {
  ...
  app.use(
    "/assets",
    express.static(path3.join(distPath, "assets"), {
      maxAge: "1y",
      immutable: true,
      etag: false,
      lastModified: false
...
// server/og-image.ts
var FONTS_DIR = path4.join(import.meta.dirname || __dirname, "fonts");
var fontDefs = [
  {
...
async function ensureFonts() {
  if (!fs3.existsSync(FONTS_DIR)) {
    fs3.mkdirSync(FONTS_DIR, { recursive: true });
  }
  for (const def of fontDefs) {
    const localPath = path4.join(FONTS_DIR, def.filename);
